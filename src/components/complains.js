...
export const Complains = () => {
  ...
  const getallcomps = async (e) => {
    ...
    for (let i = parseInt(json.history_lenght) - 1; i >= 0; i--) {
      ...
      let cmobile = json.allcomps[i].mobile
      let cdescription = json.allcomps[i].description
      let dessignal = true
      if (parseInt(cdescription.length) > 50) { dessignal = false }
  ...
  const handle = async (e) => {
    ...
    let mobilc = document.getElementById('mobilc').value
    ...
    if (Catagoryid === 'null') {
      ...
      const response = await fetch(`http://${state.backend}:${state.port}/api/c/newcomplain`, {
        ...
        body: JSON.stringify({ description: descc, catagory: Catagoryid, mobile: mobilc })
  ...
  const toastshowing = (val) => {
    ...
    if (parseInt(val) == 0) { const targetEl = document.getElementById('toast-fail'); }
    ...
    <>
      <div className="background-container">
        <div className="one two fourth justify-content-center calcby">
          ...
          <div className='complain-display relative flex flex-col min-w-0 break-words bg-white shadow-soft-xl rounded-2xl bg-clip-border h100 p-4'>
              ...
              <div className="modal-body">
                ...
                <form onSubmit={handle}>
                  <div className="grid gap-6 mb-6 md:grid-cols-2 d-flex flex-column">
                    ...
                    <div className="mb-6">
                      <input type="number" id="mobilc" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Mobile No" required />
                    </div>
                  </div>
